...
(function(root){
    ...
    es.updateType = function(uid,quantity,callback){

        var request = {
            quantity: quantity,
        }

        req('put','/cart/'+Config.cart+'/types/' + uid,request,{},function(err,res){
            if(err){
                callback(err,null);
                return;
            }
    ...
    es.getCart = function(callback) {

        var request = {}

        req('get','/cart/'+Config.cart,request,{},function(err,res){
            if(err){
                callback(err,null);
                return;
            }
    ...
    es.getFastlaneUrl = function(store){
        if(Config.cart){
            var url = 'http://checkout.staging.eventsquare.io/fastlane/'+Config.cart;
    ...
    es.helpers = {
        list: function(){
            var items = [];
            for(var s=0;s<Edition.channel.sections.length;s++){
                if(typeof Edition.channel.sections[s].data.types !== 'undefined' && Edition.channel.sections[s].data.types.length){
                    for(var i = 0; i < Edition.channel.sections[s].data.types.length; i++){
                        items.push(Edition.channel.sections[s].data.types[i]);
                    }
                }
            }
            return items;
    ...
    function loadStore(callback){
        ...
        req('get','/store/' + Config.uri,request,{},function(err,res){
